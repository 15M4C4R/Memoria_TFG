...
\chapter[Iteración 7: Auditoría y Reorganización Estructural]{Iteración 7: Auditoría Interna, Reorganización Estructural y Planificación}
...
El control de este \textit{sprint} representa un punto de inflexión crítico. La auditoría demuestra que es inasumible continuar el desarrollo sin sanear el núcleo del sistema debido al incremento de la deuda técnica por la asimetría del código (\textbf{RT-04}). Como medida de replanificación, el equipo modifica la lista de tareas, arrastrando el desarrollo del chat y priorizando una refactorización masiva para el siguiente ciclo. Se asume el riesgo consciente de que ciertas características menores queden fuera del alcance del proyecto a cambio de garantizar la integridad de la arquitectura central. A nivel de avance cualitativo, este ciclo no cierra paquetes funcionales, sino que reestructura la deuda técnica y estabiliza el código para salvaguardar la viabilidad y futura ejecución del Paquete 4 de la EDT.


\chapter[Iteración 8: Refactorización y Chat en Tiempo Real]{Iteración 8: Refactorización, Estado Inicial de Partida y Chat en Tiempo Real}
...
\section{Seguimiento, Control y Replanificación}
Durante la validación de la refactorización, el seguimiento revela brechas en el filtrado de datos, que siguen exponiendo información privada. El control de calidad fuerza a replanificar la serialización JSON, introduciendo validadores estrictos en los \textit{mappers}. Tras superar este reto, la base de código queda notablemente robusta y lista para iniciar el sistema funcional de cartas. La codificación de estos validadores supuso una carga operativa imprevista que fue absorbida mediante la compresión del cronograma en la misma semana. Se certifica el avance con el cierre de la tarea 3.4 (Implementación del chat) y de la 4.4 (Integración del patrón DTO) correspondientes a la EDT.


\chapter[Iteración 9: Motor de Juego y UX Dinámica]{Iteración 9: Desarrollo del Motor de Juego, UX Dinámica y Sincronización de Salas}
...
Tras diseñar las bases del motor de juego en esta iteración, resulta imperativo establecer cómo este nuevo sistema hereda el contexto de las iteraciones anteriores. A modo de ejemplo, se toma como referencia el caso de uso general de un usuario no logueado que accede a la plataforma para disputar un encuentro. El diagrama conceptual de la Fig.~\ref{fig:secuencia_juego} ilustra cómo queda el flujo de comunicación y la transición de responsabilidades lógicas entre todos los subsistemas del proyecto:

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/secuencia.png}
    \caption{Diagrama de Secuencia Lógico: Flujo integrado de módulos desde el acceso hasta el ciclo de partida.}
    \label{fig:secuencia_juego}
\end{figure}

\begin{enumerate}
    \item \textbf{Dominio de Autenticación y Acceso:} El usuario interactúa inicialmente con el módulo de gestión de identidad. El sistema procesa el registro o inicio de sesión y, tras autorizar el acceso, genera una identidad válida que permitirá trazar sus acciones.
    \item \textbf{Dominio Social y de Organización (\textit{Lobbies}):} Una vez cuenta con identidad, la responsabilidad se delega al módulo gestor de salas. Este componente actúa como una antesala funcional, orquestando la formación de los encuentros y manteniendo el estado de preparación de ambos contendientes.
    \item \textbf{Dominio del Motor de Reglas:} Cuando las condiciones de la sala se cumplen, el control del flujo se transfiere definitivamente al motor del juego desarrollado en el presente bloque. A partir de este momento, las decisiones estratégicas de los jugadores (como jugar una carta) son procesadas exclusivamente por este módulo, el cual altera el estado de la partida y retransmite las consecuencias a la interfaz gráfica.
\end{enumerate}
...
\section{Seguimiento, Control y Replanificación}
El control de la implementación reporta complicaciones severas al sincronizar las mecánicas asíncronas del servidor con la vista. Al aplicar reglas estrictas (\textbf{RN-03}), se originan bloqueos en la selección de cartas. La desviación obliga a replanificar el control de flujo en \textit{React}, aislando la resolución de las promesas del ciclo de vida de los modales. Tras aislar la resolución de las promesas mediante esfuerzo elástico, el ciclo logra mitigar el impacto temporal, cerrando definitivamente la tarea 3.3 (Infraestructura de salas de espera) y sentando las bases operativas de las acciones lógicas definidas en el Paquete 4 de la EDT.


\chapter[Iteración 10: Lógica de Turnos y Victorias]{Iteración 10: Lógica Base de Turnos, Interacciones de Cartas y Condiciones de Victoria}
