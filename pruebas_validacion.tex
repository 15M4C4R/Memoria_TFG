...
\begin{itemize}
    \item \textbf{Cliente (Frontend):} Se utiliza \textbf{Vitest} como motor de ejecución de pruebas por su integración nativa con Vite, junto con \textbf{React Testing Library (RTL)} para la validación de componentes basada en el comportamiento del usuario.
    \item \textbf{Servidor (Backend):} Se emplea \textbf{Jest} para la ejecución de pruebas unitarias y de integración, complementado con \textbf{Supertest} para la realización de peticiones HTTP sintéticas contra los \textit{endpoints} de la API.
\end{itemize}
...
\subsection{Validación del Cliente (Frontend)}

La validación del frontend se centra en dos pilares: la correcta renderización de componentes y la gestión del estado global mediante la \textit{Context API}.
...
\begin{lstlisting}[language=Java, caption={Prueba unitaria del componente de Login}]
...
\subsection{Validación del Servidor (Backend)}

El backend se valida mediante pruebas de integración que aseguran la consistencia de los datos y el cumplimiento de las restricciones de seguridad.
...
\begin{lstlisting}[language=Java, caption={Validación de endpoints con Supertest}]
...
\begin{lstlisting}[language=Java, caption={Prueba de lógica asimétrica de victoria}]
