...
\textbf{1. Definición de Características} \\
Durante la primera iteración, el objetivo es establecer los cimientos organizativos del proyecto, preparando las herramientas de planificación e inicializando las metodologías de trabajo en equipo. Las tareas principales consisten en la creación del calendario inicial de desarrollo y la redacción del documento de metodología para el uso de GitHub, abarcando la gestión de ramas, \textit{issues} y \textit{commits}.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
Esta iteración se centra en la creación de la infraestructura de calidad y despliegue técnico. Se requiere configurar las herramientas de \textit{testing} automatizado y establecer un entorno de Despliegue Continuo (CD) que permita visualizar los avances del proyecto en un entorno de ejecución real y accesible a través de la red.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
El requerimiento central es la creación de la página de inicio (\textit{Landing Page}) y la definición inicial de los esquemas de datos que soportan la lógica de las partidas.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
El objetivo central de esta fase es el desarrollo de una interfaz funcional que permita a los usuarios interactuar con el sistema de salas, permitiendo la visualización de los espacios disponibles, así como la creación de nuevos entornos de juego o la unión a los existentes. Adicionalmente, surge la necesidad de implementar una capa de abstracción para la base de datos mediante un sistema de mapeo objeto-relacional (ORM), lo que representa un requerimiento técnico adicional para facilitar el manejo de la persistencia.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
Esta iteración se centra en la implementación de la lógica interna de la sala de espera (\textit{Lobby Room}), permitiendo a los usuarios interactuar con el entorno antes del inicio de la partida. Los requisitos funcionales incluyen la capacidad de unirse a salas, actualizar el estado de preparación (listo/no listo) y abandonar la estancia. Sin embargo, debido a que el sistema de gestión de cuentas se encontraba en desarrollo simultáneo, se deben emplear identificadores de usuario estáticos de forma temporal para avanzar en la lógica de las salas.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
El hito principal de esta iteración es establecer la base técnica y arquitectónica de la partida mediante comunicación bidireccional en tiempo real. Se busca crear los cimientos tanto en el \textit{frontend} como en el \textit{backend} para que el estado del juego se sincronice automáticamente entre los dispositivos de ambos jugadores cada vez que se realice una acción.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
El hito principal de esta fase es la estabilización y el saneamiento de las tecnologías integradas hasta la fecha. Debido a que la configuración inicial del entorno de despliegue se realizó de manera prematura, se hace necesario revisitar esta infraestructura para garantizar un funcionamiento correcto y robusto. Asimismo, se plantea la implementación de un canal de comunicación de texto en tiempo real (chat) exclusivo para los jugadores dentro de una misma sesión de juego, con el fin de mejorar la interactividad.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
El objetivo principal de esta iteración se centra en el pulido de la experiencia de usuario (UX) y en la corrección de errores arrastrados desde la Iteración 5 respecto al ciclo de vida de las salas de espera. Concretamente, se establece la necesidad de configurar los botones de las cartas en el \textit{frontend} para mostrar textos de acción dinámicos basados en el estado y ubicación de la carta seleccionada. Adicionalmente, se definen tres características críticas para las salas: aplicar \textit{Socket.io} para sincronizar los estados de preparación (listo/no listo), garantizar la redirección simultánea de ambos jugadores a la pantalla de partida al iniciarla el líder, y expulsar automáticamente a cualquier usuario que abandone la sala por desconexión o cambio de URL.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
Una vez establecida la comunicación bidireccional estable, el objetivo de esta fase es la inmersión en el núcleo funcional del juego. Se definen e implementan las acciones fundamentales que componen el turno de un jugador: jugar cartas de tipo guardia y rebelde, gestionar el paso de turno, el robo de cartas y la mecánica exclusiva del Rey denominada ``Condenar a un Rebelde''. Para dar soporte a estas funcionalidades, se desarrollan diversos puntos de acceso en la API junto con sus lógicas de negocio correspondientes en las capas de controladores y servicios.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
La característica crítica a abordar es el sistema de "Acciones Pendientes": situaciones de juego donde el efecto de una carta interrumpe el flujo normal y exige que el jugador seleccione objetivos adicionales (cartas enemigas, zonas) antes de finalizar el turno.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
El objetivo de esta iteración se centra en perfeccionar el flujo de turnos mediante la automatización del robo de cartas para el bando del Rey y en el diseño conceptual de la mecánica de infiltración. Se busca que el Rey reciba una carta de forma obligatoria al finalizar sus acciones, asegurando la progresión del mazo real. Asimismo, se inicia el estudio técnico sobre cómo permitir que el bando rebelde inyecte cartas en el mazo enemigo, evaluando la viabilidad de diferentes interfaces para el usuario.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
Esta iteración está dedicada casi exclusivamente a la refactorización visual del Frontend de la partida, a la corrección de múltiples anomalías funcionales derivadas de la complejidad del motor de cartas, y a dar soporte de resolución a la mecánica crítica de la carta \texttt{DECOY}.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
La meta es endurecer la resiliencia del juego ante la inestabilidad de red de los usuarios, gestionando salidas abruptas, \textit{timeouts} y facilitando un retorno seguro a las salas o partidas en curso. Además, se busca unificar el sistema de \textit{Modales} (ventanas emergentes) en React, eliminando los modales nativos del navegador.
...
\subsubsection*{Desarrollo a cargo de Guillermo}
\textbf{1. Definición de Características} \\
Como sprint final de la infraestructura global, los objetivos apuntan a garantizar la calidad y despliegue final: implementar pruebas unitarias (Tests) sobre los módulos de salas, reparar bugs de lógica de negocio profunda y configurar correctamente los pobladores de la base de datos (\textit{seeders}) en producción.
